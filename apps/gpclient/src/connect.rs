...
pub(crate) struct ConnectArgs {
  ...
  #[arg(
    long,
    help = "Use external browser authentication. With no value, auto-select Chrome, Firefox, then system default.\nUse `default` for the system default browser, `remote` for headless servers,\nor the path to the browser executable.",
...
impl<'a> ConnectHandler<'a> {
  ...
  /// Try to connect straight to the last-used gateway with the cached portal
  /// cookie, skipping prelogin + SAML entirely. Returns `None` (falling back to
  /// the normal flow) if there is no usable cache or the gateway rejects it.
